...
- **Modelo Lockstep:** Este modelo permite una sicronizacion a la hora de enviar los imputs al servidor o al cliente, porque requiere que los jugadores envien su acciones antes de que el juego avance al siguiente estado. La ventaja principal es la sincronizacion precisa entre jugadores y bajo consumo del ancho de banda. Su mayor desventaja es que puede tener una conexion lenta o inestable ya que si un solo jugador tiene lag, todos los demas tambien lo sufren.
- **Modelo Deterministico:** Este modelo permite que todos los jugadores tengan una misma simulacion en sus propias computadoras y el servidor (O la red) intercambia las acciones de los jugadores en vez de constantemente actualizar el estado del juego. El hecho de que solo se transmitan los inputs de los jugadores reduce muchisimo el trafico de red, haciendolo eficiente hasta con conexiones lentas. Las desventajas es que si una sola computadora esta fuera de sincronizacion (Desync) estara jugando a una partida distinta que los demas jugadores. Varios juegos que usan este modelo tambien usan Lockstep, lo cual trae otras complicaciones (Mencionadas anteriormente). 
